...
import { X, Package, Calendar, CheckCircle2, Clock } from 'lucide-react';
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-6 sm:p-8 relative border border-gray-100 my-8 animate-in fade-in zoom-in-95 max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Tài khoản: <strong className="text-navy">{user?.name || user?.email}</strong></p>
          </div>
        </div>
              ...
              <div key={order.id} className="border border-gray-200 rounded-2xl p-4 bg-gray-50/80 space-y-3 shadow-xs">
                ...
                <div className="flex justify-between items-center text-xs pt-1 border-t border-gray-200/60">
                  ...
                  <div className="text-right">
                    ...
                    <strong className="text-orange font-black text-sm">{order.total_price.toLocaleString('vi-VN')} VNĐ</strong>
                  </div>
                </div>
              </div>
            ))
